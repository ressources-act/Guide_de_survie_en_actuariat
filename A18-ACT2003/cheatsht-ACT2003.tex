...
\begin{align*}
\widehat{\sigma}^2 = s^2 = \frac{\sum_{i=1}^{n} \hat{\varepsilon_i}^2}{n-p'} = \frac{\sum_{i=1}^{n} (Y_i - \hat{Y}_i)^2}{n-2}
\end{align*}
...
\text{Hypothèses} & t_{obs} & C \\\hline
	\begin{aligned}
		...
		H_1 &:\hat{\beta} \neq \theta_0
	\end{aligned} &  
	\frac{\hat{\beta} - \theta_0}{\sqrt{\widehat{Var(\hat{\beta}})}} \overset{H_1}{\sim} T_{(n - 2)}  &
	| t_{obs} | > \left| t_{(n - 2), \frac{k}{2}} \right| \\
\hline
\end{tabular}
\\
...
\end{enumerate}
$\because \esp{\epsilon} = 0  \therefore \widehat{\esp{Y | x_o }} = \widehat{Y}_o = \beta_0 + \beta_1 x_0$ 
...
\begin{enumerate}
	\item \textbf{Parameter risk} pour $\esp{Y | x_o }$ et $Y_0$. \\
		\textit{alias incertitude liée à l'estimation des paramètres } $\beta_0 \text{ \& } \beta_1$.
	\item \textbf{Process risk} pour $Y_0$. \\
		\textit{alias fluctuation des valeurs de la variable réponse autour de sa moyenne} $\epsilon$.
\end{enumerate}

\subsubsection*{Intervalles de confiance de niveau $1 - \kappa$}
\begin{align*}
\esp{Y | x_o } &: \Bigg[ \widehat{Y}_0 \pm t_{(n - 2), \frac{k}{2}} \sqrt{s^2 \bigg( \frac{1}{n} + \frac{(x_0 - \bar{x})^2}{S_{XX}} \bigg)} \Bigg] \\
Y_0 &: \Bigg[ \widehat{Y}_0 \pm t_{(n - 2), \frac{k}{2}} \sqrt{s^2 \bigg( 1 + \frac{1}{n} + \frac{(x_0 - \bar{x})^2}{S_{XX}} \bigg)}\Bigg]
\end{align*}

\subsection*{Analyse de la variance (ANOVA)}
\textbf{Pour déterminer la proportion de la variabilité de $Y$ est expliquée par le modèle}\\
...
Où $p$' est le nombre de variables estiméees dans le modèle.\\
...
\begin{tabular}{| >{\columncolor[gray]{.8}}c |c |}
\hline
$E[\widehat{\epsilon}_i]$ & $0$ \\\hline
$V(\widehat{\epsilon}_i)$ & $\sigma^2(1 - h_{ii})$ \\\hline
$Cov(\widehat{\epsilon}_i, \widehat{\epsilon}_j)$ & $-\sigma^2 \bigg( \frac{1}{n} + \frac{(x_i - \bar{x})(x_j - \bar{x})}{S_{xx}}\bigg)$ \\\hline
\end{tabular} \\
...
À noter que puisque $SST^{(0)} = SST^{(1)}$ on peut réécrire $F_{obs}$ comme :
\[
	F_{obs}= \frac{(SSR^{(1)} - SSR^{(0)})}{\Delta dl \  MSE^{(1)}}
\]
