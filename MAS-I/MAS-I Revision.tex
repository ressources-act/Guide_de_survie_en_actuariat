...
$\hspace*{1.5cm} \mathrm{SST} = \mathrm{SSE} + \mathrm{SSTR} + \mathrm{SSB}$
   \begin{center}
      \begin{tabular}{llc}
        \hline
        Model & Sum of square (DF)& Deviance \\
        \hline
         $Y = \mu + \varepsilon_{ij}$ & SST$(bk-1)$ & $D_M$ \\
         $Y = \mu + \alpha_i + \varepsilon_{ij}$ & SSTR $(k-1)$& $D_A$ \\
         $Y = \mu + \beta_j + \varepsilon_{ij}$ &SSB$(b-1)$& $D_B$ \\
         $Y = \mu +\alpha_i +  \beta_j + \varepsilon_{ij}$ & SSE$(k-1)(b-1)$ & $D_I$ \\
         \hline
      \end{tabular} 
   \end{center} 
\begin{itemize}[align=left,leftmargin=*]
...
\section*{Measures of Fit II}
For \textbf{contingencies table}  with binomial or poisson distribution.
\begin{itemize}[align=left,leftmargin=*]
   \item Deviance: $D = 2 \sum O_i \ln \frac{O_i}{E_i}$ 
   \item Pearson: $\chi^2 = \sum \frac{(O_i - E_i)^2}{E_i}$
   \item Likelihood ratio chi-square: $C = 2[\ell - \ell_{\mathbf{min}}]$
