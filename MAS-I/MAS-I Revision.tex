...
\begin{tikzpicture}
    \begin{axis}[
        ...
        axis y line=left,
        xtick={-1.5,1.5},
        xticklabels={$z_{\frac{\alpha}{2}}$,$z_{1 - \frac{\alpha}{2}}$},
        ytick=\empty,
        xmin = -3,
        xmax = 6,
        ymin = 0,
        ...
        (-1.8, 0.0789501583008942)
        (-1.7, 0.094049077376887)
...
\item The \textbf{p-value} is the probability of the observation if $H_0$ is true. $H_0$ is rejected if and only if the p-value is less then the signifiance level. \\ $\hspace*{2cm} P_{\mathrm{value}}<\alpha$
\end{itemize}
...
\begin{itemize}[align=left,leftmargin=*]
  \item The \textbf{chi-sqare} distribution is a one-parameter family distribution. In definition, it a gamma with $\alpha=\frac{n}{2}$ and $\theta=2$. The only parameter $n$ is called \textbf{degree of freedom}.
  \begin{itemize}
