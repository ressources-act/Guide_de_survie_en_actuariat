...
% $   \esp{\textbf{Total cost}} = \sum\limits_{i=1}^{\min(n,k)} \min(c_{i,1} + c_{i,2} + ... + c_{i,\max(n,k)}) $ \\
   \esp{\textbf{Total cost}} &= \sum\limits_{i=1}^{\min(n,k)} \frac{\theta}{\max(n,k)}
\end{align*}

\section*{Potential outliers}
Let $Q_1$ and $Q_2$ be the 25\up{e} and the 75\up{75} quantile. Then $h=1.5(Q_3 - Q_1)$. Observation are
