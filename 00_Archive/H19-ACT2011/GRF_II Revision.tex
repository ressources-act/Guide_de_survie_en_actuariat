...
\input{../../02_Cheatsheets/cheatsht-preamble-general.tex}
...
\begin{definition}[Produits dérivés]
...
\begin{definition}[Contrat à terme]
...
\begin{itemize}
	...
	\item	à un \textit{certain} \textbf{prix};
...
\begin{multicols*}{2}
\begin{itemize}
	\item[$S_{t}$:] \textbf{Prix du sous-jacent} à $t$;
	\item[$S_{0}$:] \textbf{Prix au comptant};
	\item[$F_{0, T}$:]	est le \textbf{prix à terme};
	\item[$F_{0, 0}$:]	est nul;
\end{itemize}
\end{multicols*}
La notation $F_{0, T}$ vient de \og future \fg{} ou \og forward \fg{}.
Le prix au comptant représente le paiement pour la livraison immédiate à $t = 0$.
\end{distributions}
\end{definition}

%%%	----
%%%	turn into a table with long and short
%%%	----
\paragraph{Positions}: Dans le cas de contrats à terme, les positions sont plus spécifiques:
\begin{description}
	\item[longue:]	Personne qui s'engage à \textbf{acheter};
	\item[courte:]	Personne qui s'engage à \textbf{vendre};
\end{description}
\begin{distributions}[Valeurs des positions]
\begin{multicols*}{2}
\begin{description}
	\item[longue:]	$S_{T} - F_{0, T}$;
	\item[courte:]	$F_{0, T} - S_{T}$;
\end{description}
\end{multicols*}
\end{distributions}

\begin{definition}[Option d'achat]
Contrat qui: 
\begin{itemize}
...
\paragraph{Exercice (levée):} Décision d'utiliser l'option d'achat;

\begin{conceptgen}{Types d'exercices}
\begin{description}
	\item[Européen:]		\textbf{À} $T$;
	\item[Américain:]	\textbf{N'importe quand} d'ici $T$;
	\item[Bermudien:]	À \textbf{quelques dates} d'ici $T$;
\end{description}
...
%%%	turn into a table with long and short
%%%	----
\paragraph{Positions}: Dans le cas d'option d'achat, les positions sont:
\begin{description}
	\item[longue:]	Acheteur, ou \textbf{Détenteur}, de l'action;
	\item[courte:]	\textbf{Vendeur} de l'action;
\end{description}

\begin{definition}[Option de vente]
