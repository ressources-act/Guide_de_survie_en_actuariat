...
\begin{align*}
	a(t) 
		&= 	(1 + i)^{t} 
		= 	(1 - d)^{-t}		\\
	&= 	e^{\int_{0}^{t} \delta_s ds} % va avec a(t)	
\end{align*}
\end{minipage}
\begin{minipage}{0.5\columnwidth}
\begin{align*}
...
%Rentes actualisées croissantes arithmétiquements
\begin{align*}
...
%\end{align*}	
%
%Rentes avec croissance géométrique
%\begin{align*}
...
\begin{itemize}[leftmargin = *]
	...
	\item	On dénote \icbox{$\Pr(K_x = k)	=	\px[k|]{x}$}.
\end{itemize}
\end{definitionNOHFILLsub}
...
\begin{definitionNOHFILLsub}[Durée de vie entière future restante en $m^{\text{e}}$ d'années $H^{(m)}_{x}$]
...
\begin{rappel_enhanced}[Loi de Moivre]
...
\begin{rappel_enhanced}[Loi Exponentielle]
\begin{center}
\begin{align*}
	T_x 			
	&\sim 	\mathrm{Exp}(\mu) &
...
\begin{rappel_enhanced}[Loi de Makeham]
\begin{align*}
...
\begin{rappel_enhanced}[Loi de Gompertz]
	\setlength{\mathindent}{-1cm}
\begin{align*}
...
\begin{rappel_enhanced}[Loi de Weibull]
\begin{align*}
...
On peut généraliser l'interpolation pour un intervalle $c$ de plus d'un an, \icbox[red][palechestnut]{$\forall t \in [0, c]$} :
\begin{align*}
	S_{X}(x + t) 
...
Sous l'hypothèse de \textbf{DUD}, on pose que la mortalité au courant d'une année est uniforme; la probabilité de décéder est la même peu importe le moment de l'année. Puisque $R_{x}$ est la mortalité au courant de l'année, il s'ensuit qu'elle est indépendante de $K_{x}$; alias, \icbox[red][palechestnut]{$K_{x} \overset{DUD}{\bot} R_{x}$}.

Sous l'hypothèse de la \textbf{force constante}, on peut penser à la force de mortalité comme une fonction en forme d'escalier. Elle peut uniquement changer aux années entières. Alors, $K_{x}$ et $R_{x}$ seront uniquement indépendantes si la probabilité de décès est la même peu importe l'âge atteint (par exemple, si la mortalité suit une loi géométrique). Dans ce cas rare, on dit que \icbox[red][palechestnut]{si $\px{x + k}	=	\px{x}, \ \forall k \in \{0, 1, ... \}$} alors \icbox[red][palechestnut]{$K_{x} \overset{FC}{\bot} R_{x}$}.
...
On cherche donc le moment $t$ qui va maximiser $f_{T_{x}}(t) \equiv \px[t]{x} \mu_{x + t}$, alias \icbox[red][palechestnut]{$\argmax_t \px[t]{x} \mu_{x + t}$}.\\
