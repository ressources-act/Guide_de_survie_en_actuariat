...
\begin{definitionNOHFILLsub}[Intervalle de confiance sur la moyenne]
...
\begin{description}
	\item[$p$]	La \textbf{valeur $p$} du test.
		\begin{itemize}
		\item	On dit souvent que c'est la \textbf{taille} de la région critique $\mathcal{C}$.
		\end{itemize}
	\item[Probabilité d'une erreur de type I]	Probabilité de rejeter $\text{H}_{0}$ alors qu'elle est vraie.
		\begin{itemize}
		\item	Même chose que la valeur $p$.
		\end{itemize}
	\item[$\alpha$]	\textbf{Seuil de signifiance} du test.
		\begin{itemize}
		\item	Même chose que pour les intervalles de confiance;
		\item	Essentiellement, c'est la valeur $p$ qui détermine si on rejette ou pas un test;
		\item	En anglais, \og \textit{threshold for significance} \fg{}.
		\end{itemize}
\end{description}
\end{distributions}

Formellement, on défini \lfbox[formula]{$\alpha	=	\underset{\theta \in \Theta_{0}}{\max} \Pr\left\{(X_{1}, \dots, X_{n}) \in \mathcal{C} ; \theta \right\}$}. \\
C'est-à-dire :
\begin{itemize}
...
%%%	qqchose que je manque ici
%%%	\Theta vs S, test vs param?? (X, ... X_n) \in ...?
