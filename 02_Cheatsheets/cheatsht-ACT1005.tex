...
\begin{itemize}[leftmargin =  *]
	\item	Pension de la sécurité de la vieillesse (\textit{PSV});
	\item	Supplément du revenu garantie (\textit{SRG});
...
\begin{description}
	\item[1952]	Entrée en vigueur de la loi de la sécurité et vieillesse;
		...
		\item	Idée de garantir à \textbf{tous}	les citoyens âgés (70 ans et plus) une pension non-liée au revenu avec la PSV.
		\end{itemize}
...
%%%	--------------
%%%	NOTE
%%%	+	mis en commentaire, je ne sais pas à quel point c'est pertinent de mentionner (AJVR)
%%% +   Je trouves ça pertinent. (OC)
%%%	--------------
		\end{itemize}
...
%		\item	Initialement supposé être temporaire pour aider ceux déjà à la retraite, car le \textit{RCP} et \textit{RRQ} n'allaient pas payer de prestations avant 10 ans;
		\item	\textbf{Diminution de l'âge d'admissibilité à 65 ans};
...
%%%	--------------
	\item[1975]	Entrée en vigueur de l'Allocation au conjoint ;
		\begin{itemize}[leftmargin = *]
		\item	Lorsqu'un seul membre du couple reçoit la \textit{PSV} et le \textit{RSG} et que l'autre a entre 60 et 64 ans;
...
\begin{itemize}[leftmargin = *]
	...
	\item	Pas de test de besoin, mais possibilité de devoir la rembourser (impôt de récupération);
...
\begin{itemize}[leftmargin = *]
	...
	\item	Dont au moins 10 ans immédiatement avant la demande \textbf{ou} 1 an de résidence avant la demande et 3 ans de résidence avant l'âge de 55 ans pour chaque année manquante du 10 ans; 
\end{itemize}
...
\begin{description}
	...
	\item[Report] En échange d'une pension plus élevée, il est possible depuis juillet 2013 de reporter la PSV jusqu'à 5 ans;
		...
		\item   L'impôt de récupération explique pourquoi certaines personnes pourraient vouloir reporter leur PSV (\textcite{red}{expliqué plus tard}).
		\end{itemize}
...
\begin{conceptgen}{Âge à la retraite}
	\begin{itemize}[leftmargin = *]
		...
		\item	La raréfaction de la main d'oeuvre ca mettre une pression sur les travailleurs de rester sur le marché du travail plus longtemps;
...
\begin{conceptgen}{Financement}
	\begin{itemize}[leftmargin = *]
		\item	Le financement de la \textit{PSV} est \og pay as you go \fg{}, il n'y a pas de caisse distincte et sort directement des impôts.
				\begin{itemize}
...
\begin{definitionNOHFILL}[Définition]
Prestation mensuelle non imposable offerte aux bénéficiaires de la \textit{PSV} ayant un faible revenu résidant au Canada. Tel que mentionné précédemment, le \textit{SRG} est entré en vigueur en 1967 à la suite de la mise en place du \textit{RPC/RRQ} (1966).
\end{definitionNOHFILL}

\begin{conceptgen}{Admissibilité}
\begin{itemize}[leftmargin = *]
	\item	Recevoir la \textit{PSV}--- le \texit{SRG} peut être reçu en même temps que la \texit{PSV};
...
Montants maximaux mensuels (\textit{1^{er} Janvier 2020}):
\begin{itemize}[leftmargin = *]
	\item	Personne seule et personne ayant un conjoint qui ne reçoit pas la \textit{PSV} : 916.38\$;
	\item	Personne ayant un conjoint qui reçoit la \textit{PSV}  ou l'Allocation : 551.63\$;
\end{itemize}
...
\begin{description}
	...
	\item[Financement]	Par répartition : \textit{Pay as you go};
\end{description}
\end{conceptgen}
...
%%%	--------------
%%%	NOTE
%%%	+	Faudrait développer ces deux sous-sections d'allocations---j'étais tanné (AJVR)
%%% +   Je m'en occupe ! (j'essaye) (OC)
%%%	--------------
...
\begin{definitionNOHFILL}[Définition]
Prestation mensuelle payable à une personne entre 60 et 64 ans veuf (ou veuve). (existe depuis 1985)
\end{definitionNOHFILL}
...
\begin{itemize}[leftmargin = *]
	...
	\item	Il y a des particularités sur la réduction du paiement.
		\begin{itemize}
		\item   Allocation réduite selon les revenus autres que la \textit{PSV}, \textit{SRG} de 3\$ par 4\$ de revenus jusqu'à ce que le montant soit égal à la \textit{PSV};
		\item   Réduit de 1\$ par 2\$ de revenu ensuite. 
		\end{itemize}
\end{itemize}
\end{conceptgen}
...
\begin{conceptgen}{Cessation des paiements}
\begin{itemize}[leftmargin = *]
	\item 	Pas de déclaration de revenu
