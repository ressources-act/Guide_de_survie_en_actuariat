...
\begin{itemize}
	...
	\item	La \textbf{gravité} des effets (ou conséquences) de l'événement;
\end{itemize}
...
\begin{itemize}
	...
	\item	Une compagnie d'assurance de voyage assure une personne contre le \textbf{danger} d'aller au Mexique;
%%%	-----
%%%	NOTE:
%%%	+	Définir plus précisement ce que je veux dire par danger---trops vague;
%%%	+	Danger de se faire voler? Tuer? Je ne suis pas certain;
%%%	-----
\end{itemize}
\end{definition}

\begin{definition}[Aversion]
...
Le \textbf{degré d'aversion} au risque est \textbf{variable} selon l'intervenant. Pour exemple, même les compagnies d'assurance se \textit{\textbf{ré}}assurent. 
...
\begin{itemize}[leftmargin = *]
	\item	Évitement;
	\item	Prévention;
	\item	Prise de risque (\textbf{rétention});
	\item	Transfert;
	\item	Diversification des risques;
	\item	Couverture des risques (\textbf{hedging});
	\item	La titrisation;
\end{itemize}
\end{multicols*}
%
...
Les assureurs protègent contre un risque avec la \textit{loi des grands nombres}:
\begin{itemize}
...
\begin{itemize}
	...
	\item	\textit{Transfère} le risque de l'assuré à un, ou plusieurs, autres;
...
\end{itemize}
Ainsi, il obtient une deuxième source de revenus.
\end{algo}

\begin{formula}{Exemples de risques}
...
\end{itemize}
%\end{multicols*}
En assurance IARD pour les \textcolor{amethyst}{individus} et \textcolor{amaranth}{entreprises}:
\begin{itemize}[leftmargin = *]
