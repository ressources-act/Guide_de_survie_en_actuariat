...
%ce serait bien d'avoir des diagrammes de Venn pour des exemples, ce serait plus visuel et surtout moins encombré FC
  ...
  \item[L'intersection ({$\cap$}) :] On peut le définir par un et. 
    ...
    \end{itemize}
  \item[Complémentaire :] Un événement quelconque est le complémentaire d'un événement A lorsqu'il correspond à tous les résultats de $\Omega$ excluant les résultats de A. Le complémentaire d'un événement A est désigné par {$\overline{\rm A}$}, $A^{c}$ et $A^{t}$.
    \begin{itemize}
