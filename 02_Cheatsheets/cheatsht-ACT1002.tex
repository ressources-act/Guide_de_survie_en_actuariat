...
\end{description}  
\begin{description}
  \item[Solutions entières non-négatives :] Le nombre de façons dont on peut distribuer un nombre d'objets indissociables dans des «contenants». Il peut y avoir aucun objet dans un «contenant». La solution à ce type de problème est donnée par $\binom{n+r-1}{r-1}$ où n est le nombre d'objets et r le nombre de «contenants». S'il est mentionné dans le problème qu'il n'est pas nécessaire d'utiliser tous les objets pour les mettre dans les contenants, on peut rajouter un contenant pour les objets «non-utilisés».
...
%ce serait bien d'avoir des diagrammes de Venn pour des exemples, ce serait plus visuel et surtout moins encombré FC
  ...
  %qqn peut replacer cette ligne juste en haut svp FC
  ...
  \item[Loi de DeMorgan :] ($\bigcup_{i=1}^{n} A_i$)^{c} = ($\bigcap_{i=1}^{n} A_i^{c}$)
