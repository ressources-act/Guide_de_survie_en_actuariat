...
\begin{enumerate}
	\item	Acheter $\Delta	=	e^{-\delta h} \left(\frac{V_{u} - V_{d}}{S_{0}(u - d)}\right)$ parts de l'action;
	\item	Prêter $B	=	e^{-\delta h} \left[ V_{d}\left(	\frac{u}{u - d}\right) - V_{u} \left(\frac{d}{u - d}\right) \right]$ au taux sans risque;
\end{enumerate}
...
\begin{description}[leftmargin = *]
	...
	\item[$h$]	durée de chacune des périodes;
		...
		h	=	\frac{T}{n}
		\end{align*}
%	\item[$VAE$]	Valeur espérée actualisée.
%	\item[$VSL$]	Valeur si levée.
\end{description}
\end{distributions}
...
\tcbline
\begin{itemize}[leftmargin = *]
	\item	La probabilité d'atteindre un nœud terminal est calculé avec une distribution binomiale;
