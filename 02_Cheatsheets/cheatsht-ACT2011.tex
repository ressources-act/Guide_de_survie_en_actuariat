...
\begin{itemize}[leftmargin = *]
	...
	\item	Cependant, la meilleure approximation se fait en incluant Theta pour considérer \textit{l'effet du temps} \icbox[red][palechestnut]{mais ce n'est pas dans le cadre du cours}.
\end{itemize}
...
\begin{description}
	...
	\item[$G$]	Vrai prix de l'option utilisant la moyenne géométrique selon la formule connue;
		\begin{itemize}
		\item	On peut donc obtenir l'erreur \lfbox[conditions]{$G	-	\overline{G}$}.
		\end{itemize}
	\item[$A^{*}$]	Estimé ajusté du prix de l'option utilisant la moyenne arithmétique.
		\begin{align*}
		A^{*}
		&=	\overline{A}	+	\beta	(G	-	\overline{G})
		\end{align*}
		\begin{itemize}
%		\item	On trouve que $\text{Var}(A^{*})	=	\text{Var}(A) + \beta^{2}\text{Var}(\overline{G})	-	2\beta\text{Cov}(\overline{A}, \overline{G})$
		\item	On trouve que $\text{Var}(A^{*})	=	\text{Var}(A)(1	-	\rho^{2})$ où $\rho	=	\text{Corr}(\overline{A}, \overline{G})$;
		\item	On trouve que $\hat{\beta}	=	\frac{\text{Cov}(\overline{A}, \overline{G})}{\text{Var}(\overline{G})}$.
		\end{itemize}
\end{description}
\end{distributions}
