...
Tout comme un couteau n'est pas dangereux de soi, les produits dérivés ne le sont pas non plus. Cependant, on peut heurter quelqu'un avec un couteau tout comme on peut couper des patates. Les produits dérivés sont en fait des \textbf{outils de gestion du risque} qui deviennent utiles lorsque le risque du sous-jacent augmente.
...
\begin{description}
	...
	\item[Teneur de marché]	intermédiaire visant à faire un profit de la transaction entre end-users;
		\begin{itemize}[leftmargin = *]
		\item	\og \textit{Market-makers} \fg{}.
		\end{itemize}
	\item[Economic observers]	observateurs du marché analysant et régulant les activités des teneurs de marché et end-users.
\end{description}
\end{conceptgen}
...
\begin{description}
	\item[Risque de défaut (de crédit)] Risque de ne pas être payé;
		...
		\end{itemize}	
	\item[Risque de rareté] S’il est difficile de trouver un acheteur et un vendeur pour le sous-jacent (peu de transactions signifient beaucoup de négociations et de variation dans les prix).
\end{description}
\end{conceptgen}
...
\begin{description}
	...
	\item[Valeur à l'échéance $T$:]	Les flux de trésorier au temps $t = T$;
		\begin{itemize}[leftmargin = *]
		...
		\end{itemize}
	\item[Profit\footnote{\textbf{V}aleur \textbf{A}ccumulée au taux sans risque.}] 
		= (valeur à l'échéance) - (valeur accumulée du coût initial) \\
		= $VA$(flux monétaires)\footnote{Par exemple acheter une voiture et la revendre 10 ans plus tard---(prix de revente à dans 10 ans) - (valeur accumulée du coût d'achat initial à 0 au courant des 10 dernières années).}
%	\begin{align*}
%	&=	\begin{cases}
...
\end{description}
En réalité, la \textit{majorité sont européens} et donc nous effectuons uniquement des calculs avec ce type.
\end{conceptgen}
\end{definitionNOHFILL}
...
On achète une option de vente pour garantir un prix minimum de vente. On dit donc avoir une option de vente de protection qui sert de \textit{plancher} minimale.
...
On achète une option d'achat pour garantir un prix maximal d'achat. Ce faisant, on \textit{plafonne} le prix d'achat.
