...
\begin{itemize}[leftmargin = *]
	\item	De façon similaire à l'écart papillon, on vend et achète en proportion différentes le stellage et stellage élargi;
	\item	$\text{A-BFS} = mCall(K_1, T) - (m + n) Call(K_2, T) + n Call(K_3, T)$;
	\item	L'écart est asymétrique avec $m(K_{2} - K_{1}) = n(K_{3} - K_{2})$.
\end{itemize}

On utilise la notation avec $\lambda$: 
\begin{align*}
	\lambda	&=	\frac{K_{3} - K_{2}}{K_{3} - K_{1}}
\end{align*}
Pour chaque option avec un prix d'exercice de $K_{2}$ achetée, on achète $\lambda K_{1}$ et $(1 - \lambda) K_{3}$.
\end{definitionNOHFILL}
...
\begin{itemize}
	...
	\item	Si le prix d'exercice baisse de, par exemple, 10\$, alors la valeur à l'échéance maximal possible augmente de 10\$;
	\item	Ce faisant, le plus que je sera prêt à payer pour potentiellement avoir 10\$ de plus \textit{est} 10\$ de plus.
\end{itemize}
...
\begin{description}
	...
	\item[Option de vente]	
		\begin{align*}
		C_{\text{amer}}(K_{2}) - C_{\text{amer}}(K_{1}) &\leq K_{2} - K_{1}	\\
		C_{\text{euro}}(K_{2}) - C_{\text{euro}}(K_{1}) &\leq (K_{2} - K_{1})\textrm{e}^{-rT}
		\end{align*}
\end{description}
