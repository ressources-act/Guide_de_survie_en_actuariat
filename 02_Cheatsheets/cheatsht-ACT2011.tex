...
\begin{definitionNOHFILL}[Moyenne variance et covariance]
...
\begin{definitionNOHFILL}[Prix d'options européennes]
\begin{align*}
	C(K)
...
\end{center}
\begin{itemize}[leftmargin = *]
	\item	Le rendement composé continûment sur l'action a une distribution normale et est indépendant avec le temps.
	\item	La volatilité est connue et constante.
	\item	Les futurs dividendes sont connus.
\end{itemize}
\tcbline
\begin{center}
...
\end{center}
\begin{itemize}[leftmargin = *]
	\item	Le taux sans risque est connu et constant.
	\item	Il n'y a pas d'impôt	 ni de frais de transactions.
	\item	Il n'y a aucun coût relié à la vente à découvert.
...
\begin{description}
	...
	\item[Devises]	Les prix à terme prépayés doivent être dans la même devise que l'option.
		\begin{align*}
		C(x_{0}, K, \sigma, r_{\text{DD}}, T, r_{\text{DÉ}})
...
\begin{itemize}[leftmargin = *]
	...
	\item	Vendant l'option d'achat au temps $t_{2}$
\end{itemize}
...
\begin{description}
	\item[Valeur à $t_{2}$]	
		\begin{align*}
		&=	\text{Prix de revente}	\\
		&=	C(S_{t_{2}}, K, \sigma, r, T - t_{2}, \delta)
		\end{align*}
	\item[Profit à $t_{2}$]	
		\begin{align*}
		&=	\text{Prix de revente} - \text{Prix d'achat à $t_{1}$ accumulé à $t_{2}$}	\\
		&=	C(S_{t_{2}}, K, \sigma, r, T - t_{2}, \delta) - C(S_{t_{1}}, K, \sigma, r, T - t_{1}, \delta) \textrm{e}^{r(t_{2} - t_{1})}
		\end{align*}
\end{description}
\end{definitionNOHFILL}
...
\begin{itemize}[leftmargin = *]
	...
	\item	\og \textit{calendar spread} \fg{}.
\end{itemize}
\end{definitionNOHFILL}
...
%%%	---------------
%%%	NOTE
%%%	+	Manque volatility skew, etc.
%%%	+	Plus de détails sur le contexte général.
%%%	+	Utilité.
%%%	---------------

\begin{itemize}[leftmargin = *]
	\item	La volatilité n'est pas observable et sont estimation est problématique.
	\item	La volatilité \textit{historique} ne prévoit pas le futur.
	\item	On peut donc isoler la volatilité \textit{implicite} avec le prix observé de l'action et la formule de Black-Scholes.
%	\item	Malgré que la volatilité est supposée constante dans la formule, elle varie avec le prix d'exercice $K$ et la date d'échéance $T$.
...
\end{itemize}
\begin{description}
	\item[Volatilité implicite]	La valeur de $\sigma$ telle que la formule de Black-Scholes reproduit le prix de l’option observé sur le marché.
\end{description}

%%%	--------------------
%%%	PAS CADRE DU COURS
%\subsection*{Perpetual american options}
%\hl{à compléter}
%%%	--------------------
...
\tcbline
%Lien entre delta d'options de vente et d'achat:
%\begin{align*}
...
\begin{definitionNOHFILL}[Gamma $\Gamma = \deriv{S}{\Delta} \equiv \deriv[2]{S}{V}$]
...
\begin{align*}
	\Gamma_{C}
	&=	\Gamma_{P}
	=	\textrm{e}^{-\delta T} N'(d_{1}) \frac{1}{S \sigma \sqrt{T}}	
\end{align*}
\end{gather}
\end{minipage}
\begin{minipage}[ht]{0.5\linewidth}
...
\begin{itemize}[leftmargin = *]
	...
	\item	Le $\Gamma$ est maximisé au cours du marché car c'est où le $\Delta$ connait le plus de variation.
		\begin{center}
		\includegraphics[scale=0.4]{src/ACT-2011/gamma-delta-greek.png}
		\end{center}
\end{itemize}
\end{definitionNOHFILL}
...
\begin{align*}
	\theta_{C}
	&=	\delta S \textrm{e}^{-\delta (T - t)} N(d_{1}) - 
		r K \textrm{e}^{-r (T - t)} N(d_{2})	-
		\frac{K\textrm{e}^{-r (T - t)} N'(d_{2}) \sigma}{2\sqrt{T - t}}	\\
	\theta_{P}
	&=	\theta_{C} + 
		r K \textrm{e}^{-r (T - t)}	-
		\delta S \textrm{e}^{-\delta (T - t)} 
\end{align*}
\end{gather}
...
\begin{itemize}[leftmargin = *]
	...
	\item	Pour obtenir la variation en valeur par jour, il suffit de diviser $\theta$ par 365.
	\item	\textbf{T}heta pour \textbf{t}emps.
\end{itemize}
\end{definitionNOHFILL}

\begin{definitionNOHFILL}[Vega $Vega	=	\deriv{\sigma}{V}$]
...
\begin{align*}
	Vega_{C}
	&=	Vega_{P}
	=	S \textrm{e}^{-\delta T} N'(d_{1}) \sqrt{T}
\end{align*}
\end{gather}
...
\begin{itemize}[leftmargin = *]
	...
	\item	Pour obtenir la variation en pourcentage, il suffit de diviser $Vega$ par 100.
...
\begin{itemize}[leftmargin = *]
	...
	\item	Pour obtenir la variation en pourcentage, il suffit de diviser $\rho$ par 100.
...
\begin{itemize}[leftmargin = *]
	...
	\item	Pour obtenir la variation en pourcentage, il suffit de diviser $\psi$ par 100.
\end{itemize}
\end{definitionNOHFILL}
...
Le grecque d'un portefeuille composé de $N$ options sur une même action, avec $n_{i}$ options de chaque type, est simplement la somme des grecques:
\begin{align*}
	\text{Grec}_{\text{ptf.}}
...
\end{center}
\begin{itemize}[leftmargin = *]
	\item	Pour obtenir l'élasticité par augmentation de 1\% du prix de l'action, diviser par 100---$\Omega/100$.
	\item	Le $\Delta$ peut être vu comme $\Delta = \frac{\text{variation du prix de l'option}}{\text{variation du prix de l'action}}$ et donc l'Omega évalue le \% au lieu du montant.
\end{itemize}
\end{definitionNOHFILL}
...
\begin{description}
	...
	\item[Prime de risque sur l'option]	$\gamma - r \equiv \Omega (\alpha - r)$
\end{description}
...
\begin{description}
	...
	\item[Ratio de Sharpe de l'option]	
		...
		\equiv	\frac{\Omega}{|\Omega|}\sigma_{\text{option}}	
		\end{align*}
\end{description}
Puisque $\Omega$ est positif pour les option d'achat et négatif pour les options de vente:
\begin{align*}
...
L'élasticité d'un portefeuille composé de $n$ options est simplement la moyenne pondérée de l'élasticité de chaque type d'option. Donc, avec $\omega_{i}$\% du portefeuille investi dans l'option $i$, on a:
\begin{align*}
	\Omega_{\text{ptf.}}
	&=	\overset{n}{\underset{i = 1}{\sum}} \omega_{i} \Omega_{i}, \: \text{où } \omega_{i} = \text{\# acheté} \times \frac{V_{i}}{V_{\text{ptf}}}
\end{align*}
...
\begin{itemize}[leftmargin = *]
	...
	\item	Le Delta \textit{varie avec le prix} et donc est seulement valide comme approximation du prix pour des très petites variations. \\
			En considérant le Gamma, l'approximation devient valide pour des plus grandes variations.
...
\begin{align*}
	V_{t + h}
	&\approx	V_{t} + \Delta_{t} \varepsilon + \frac{1}{2} \Gamma_{t} \varepsilon^{2} + \theta_{t} h	
\end{align*}
