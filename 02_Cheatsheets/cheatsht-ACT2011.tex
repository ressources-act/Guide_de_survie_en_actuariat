...
\begin{description}
	...
	\item[Valeur à l'échéance $T$:]	Les flux de trésorie au temps $t = T$;
		...
		\item	C'est à dire, l'argent que l'on reçoit à l'échéance;
		\item	Dans le cas de l'achat d'un actif, on reçoit son prix et donc s'il est nul on ne reçoit rien;
		\item	C'est le profit qui prend en compte les dépenses initiales et qui sera négatif.
		\end{itemize}
...
\draw    (485.25,196.7) -- (485.25,187.05) ;


%Straight Lines [id:da5840111395603904] 
\draw    (438.5,196.65) -- (438.5,187) ;


%Straight Lines [id:da3398468398362082] 
\draw [color={rgb, 255:red, 74; green, 144; blue, 226 }  ,draw opacity=1 ][line width=2.25]    (490.25,134.64) -- (552.69,134.69) ;


%Straight Lines [id:da766471031454568] 
\draw [color={rgb, 255:red, 74; green, 144; blue, 226 }  ,draw opacity=1 ][line width=2.25]    (453.12,166.62) -- (490.25,134.64) ;


%Straight Lines [id:da8179350208142249] 
\draw [color={rgb, 255:red, 74; green, 144; blue, 226 }  ,draw opacity=1 ][line width=2.25]    (391.45,165.92) -- (453.88,165.96) ;


%Straight Lines [id:da15703311440320822] 
\draw [color={rgb, 255:red, 208; green, 2; blue, 27 }  ,draw opacity=1 ][line width=2.25]  [dash pattern={on 6.75pt off 4.5pt}]  (489.12,120.04) -- (551.55,120.08) ;


%Straight Lines [id:da2509003548986979] 
\draw [color={rgb, 255:red, 208; green, 2; blue, 27 }  ,draw opacity=1 ][line width=2.25]  [dash pattern={on 6.75pt off 4.5pt}]  (451.99,152.01) -- (489.12,120.04) ;


%Straight Lines [id:da8299907131334008] 
\draw [color={rgb, 255:red, 208; green, 2; blue, 27 }  ,draw opacity=1 ][line width=2.25]  [dash pattern={on 6.75pt off 4.5pt}]  (390.31,151.31) -- (452.75,151.36) ;
...
\draw   (437.18,222.33) .. controls (437.18,227) and (439.51,229.33) .. (444.18,229.33) -- (451.96,229.33) .. controls (458.63,229.33) and (461.96,231.66) .. (461.96,236.33) .. controls (461.96,231.66) and (465.29,229.33) .. (471.96,229.33)(468.96,229.33) -- (479.73,229.33) .. controls (484.4,229.33) and (486.73,227) .. (486.73,222.33) ;
%Straight Lines [id:da5856705496016874] 
\draw    (385.83,121.33) -- (397.83,121.33) ;


%Shape: Brace [id:dp8659015694536814] 
\draw   (558.5,153.67) .. controls (563.07,153.67) and (565.36,151.38) .. (565.36,146.8) -- (565.36,146.8) .. controls (565.36,140.27) and (567.65,137) .. (572.23,137) .. controls (567.65,137) and (565.36,133.73) .. (565.36,127.2)(565.36,130.14) -- (565.36,127.2) .. controls (565.36,122.62) and (563.07,120.33) .. (558.5,120.33) ;
...
% Text Node
\draw (442,210) node   [align=left] {$\displaystyle K_{1}$};
% Text Node
\draw (488,210) node   [align=left] {$\displaystyle K_{2}$};
% Text Node
\draw (530,92.33) node   [align=left] {\textcolor[rgb]{0.96,0.65,0.14}{{\scriptsize actif sous-jacent}}};
% Text Node
\draw (462.67,242.33) node   [align=left] {{\scriptsize largeur du tunnel}};
% Text Node
\draw (372,124) node   [align=left] {$\displaystyle K_{2}$};
% Text Node
\draw (593,134.33) node  [rotate=-90] [align=left] {{\scriptsize largeur}};
% Text Node
\draw (583,132.33) node  [rotate=-90] [align=left] {{\scriptsize du tunnel}};
...
%Ces combinaisons bénéficient d'une 
...
\begin{itemize}
	...
	\item	vendant un stellage avec un prix d'exercice $K_{2}$ où 
	\item	$K_{1} < K_{2} < K_{3}$;
...
\begin{itemize}[leftmargin = *]
	...
	\item	Pour exemple, un écart haussier aux prix d'exercice $K_{1}$ et $K_{2}$ combiné avec un écart baissier aux prix d'exercice $K_{2}$ et $K_{3}$.
\end{itemize}
...
Lorsque la différence en prix n'est pas symétrique, on fait une "interpolation" de sorte.
\begin{enumerate}
...
\end{align*}
Pour chaque option avec un prix d'exercice de $K_{2}$ vendu, on achète $\lambda K_{1}$ et $(1 - \lambda) K_{3}$.
\end{definitionNOHFILL}
...
\begin{description}
	...
	\item[Contrat à terme de gré à gré prépayé \og \textit{Prepaid forward contract} \fg{}:]	Paye au début ($t = 0$) au prix $P$ au lieu de plus tard, mais on reçoit quand même l'actif plus tard.
\end{description}
Ce faisant, on s'attend que $F_{0, T} = F_{0, T}^{P} \textrm{e}^{rT}$.

\begin{distributions}[Notation de prix]
\begin{itemize}
	\item[$F_{0, T}^{P}$:]	est le \textbf{prix à terme} d'un contrat à terme de gré à gré prépayé;
...
Nous tarifions des contrats à terme de gré à gré \textit{prépayé} et utilisons ces prix pour dériver le prix des contrats à terme de gré à gré. 
\end{definitionNOHFILL}

\subsection*{Tarification d'un \textbf{contrat à terme de gré à gré prépayé}}

Sans dividendes, le prix du contrat à terme de gré à gré prépayé est le prix de l'actif sous-jacent aujourd'hui---$S_{0}$. Dans le cadre d'une action cependant, il peut y avoir des \textbf{dividendes}. 

Les dividendes sont payables au propriétaire de l'action. Puisque l'acheteur du contrat (position longue) va seulement posséder le contrat au temps $T$, il ne recevra alors pas de dividendes. Cela baisse donc la valeur de l'action et le prix du contrat devrai tenir cela en compte.
...
Un modèle de \textbf{dividendes payés continument} suppose un taux de dividendes continûment composé $\delta$. Alors, une part de l'action au temps initial devient $e_{\delta T}$ parts au temps $T$. Cependant, nous voulons seulement une part au temps $T$ et donc nous devons acheter $e_{-\delta T}$ parts au début. Le prix est donc $F_{0, T}^{P} = S_{0} e^{-\delta T}$.
...
\begin{itemize}
	\item	le coût inital est nul et
...
\begin{center}
	...
	\end{tabular}
\end{center}
Si la réponse à tous les questions est oui, alors il y a une opportunité d'arbitrage. Pour identifier les flux monétaires on utilise l'approche à deux étapes:
\begin{enumerate}
	\item	Écrire, sous forme d'inégalité, ce qui est observé;
	\item	Déplacer tout ce qui est sur le coté inférieur ($<$) au coté supérieur ou égale ($\ge$);
...
\begin{itemize}
	\item[$x_{t}$]	taux de change au temps $t$ en DD/DÉ
		\begin{itemize}[leftmargin = *]
		\item	Par exemple, $x_{0}\frac{2\$}{\text{1\euro}} = x_{0}\left(2 \frac{\$}{\text{\euro}}\right) \equiv x_{0}\frac{2\text{DD}}{1\text{DÉ}}$
		\end{itemize}
\end{itemize}
...
Différences aux contrats à terme de gré à gré:
\begin{enumerate}[leftmargin = *]
	\item	\textbf{Personnalisable};
		\begin{itemize}[leftmargin = *]
		\item	Pour un \textit{forward}, l'acheteur et le vendeur peuvent choisir n'importe quel prix, montant, date d'échéance ou actif sous-jacent;	
		\item	Pour un \textit{future}, ces composantes sont prédéterminées et ont dit donc qu'il est \textbf{standardisé}.
		\end{itemize}
	\item	\textbf{Valorisation au prix du marché} \og \textit{Marked-to-Market} \fg{};
		\begin{itemize}[leftmargin = *]
		\item	Pour un \textit{forward}, tous les échanges d'argent se produisent à l'échéance (contrat est réglé à l'échéance);	
		\item	Pour un \textit{future}, les pertes et profits sont réglés tous les jours (processus de valorisation au prix du marché).
		\end{itemize}
	\item	\textbf{Risque de défaut};
		\begin{itemize}[leftmargin = *]
		\item	Pour un \textit{forward}, le contrat est pleinement exposé au risque de défaut; 
		\item	Pour un \textit{future}, avec le règlement quotidien des pertes et de profit le risque de défaut est minimisé.
		\end{itemize}
	\item	\textbf{Liquidité}: fait référence à l'aise d'acheter ou de vendre un actif, ou de façon équivalente, de sortir ou entrer de leur position;
		\begin{itemize}[leftmargin = *]
		\item	Pour un \textit{forward}, il est impossible de sortir d'un contrat et donc ils ne sont pas liquides;
		\item	Pour un \textit{future}, puisqu'ils sont transigés sur les marchés boursiers, ils sont liquides.
		\end{itemize}
	\item	\textbf{Limite de prix};
		\begin{itemize}[leftmargin = *]
		\item	Pour un \textit{forward}, il n'y a aucune limite de prix sur l'actif sous-jacent et il peut varier sans limites;
		\item	Pour un \textit{future}, il y a des limites incluses dans le contrat.
		\end{itemize}
\end{enumerate}
...
\begin{description}
	...
	\item[Marge initale]	Le dépôt initial est nommé la marge initiale (\og \textit{inital margin} \fg{});
...
\begin{algo2}[Exemple]
On suppose l'achat d'une action se transigeant au prix de 950\$ aujourd'hui ayant 250 parts sous-jacent. La force d'intérêt est de 6\%, la marge initiale égale à 80\% de la valeur notionnelle et la marge de maintien 10\% de la marge initiale.
...
\begin{align*}
	\overset{\text{marge de maintien}}{\overbrace{
		...
		\underset{\text{taux de MI}}{\underbrace{0.10}} \times 
...
\paragraph{Note}	Si le prix d'exercice sera dans la même devise que le prix de l'option.
...
Une option européenne peut uniquement être exercée à l'échéance alors qu'une option américaine peut être exercé à n'importe quel moment d'ici l'échéance. Il s'ensuit qu'une \textbf{option américaine vaut au moins autant qu'une option européenne}:
\begin{align*}
...
\paragraph{Option américaine}	Une option américaine aura une valeur d'au moins l'option européenne. Également, puisqu'une option américaine peut être exercé à tout moment, sa valeur doit être au moins la valeur d'un exercice immédiat.
...
\paragraph{Option de vente}	La distinction avec la valeur maximal d'une option européenne est qu'elle est seulement reçu à $T$.\\
...
\begin{enumerate}
	\item	Il est rationnel d'effectuer un exercice hâtif si\\
 	...
 	\item[]	$PV(\text{assurance})$ sera donc égale au coût de l'option de vente équivalente.
...
\begin{enumerate}
	\item	Il est rationnel d'effectuer un exercice hâtif si\\
 	...
 	\item[]	$PV(\text{assurance})$ sera donc égale au coût de l'option d'achat équivalente.
	\item	Il est \textbf{\textit{possiblement} rationnel} d'effectuer un exercice hâtif si \\
...
Si la différence en prix d'option était supérieure à la différence en prix d'exercice il y a arbitrage; on peut avoir aucun risque de perte. Pour comprendre ceci, voir le graphique d'un écart-baissier et imaginer la ligne pointillée rouge uniquement au-dessus de l'axe des x.
...
\begin{enumerate}
	\item	Acheter $\Delta	=	e^{-\delta T} \left(\frac{V_{u} - V_{d}}{S_{0}(u - d)}\right)$ parts de l'action;
	\item	Prêter $B	=	e^{-\delta T} \left[ V_{u} \left(\frac{d}{u - d}\right) + V_{d}\left(	\frac{u}{u - d}\right) \right]$ au taux sans risque;
\end{enumerate}
...
\begin{description}[leftmargin = *]
	\item[$p^{*}$]	la probabilité neutre au risque d'une hausse de l'actif.
		\begin{align*}
		p^{*}	&=	\frac{\textrm{e}^{(r - \delta)T} - d}{u - d}
		\end{align*}
\end{description}
Le prix est donc :
	\begin{align*}
	V_{0}	&=	\textrm{e}^{-rT} \left[p^{*} V_{u} + (1 - p^{*})V_{d} \right]
	\end{align*}
\end{distributions}

On déduit que $0 < p^{*} < 1$ ce qui mène à l'équation suivante:
	\begin{align*}
	d < \textrm{e}^{(r - \delta)T} < u
	\end{align*}
...
\begin{description}[leftmargin = *]
	...
	\item[]	L'écart type de la volatilité sur une période de durée $T$ est donc $\sigma \sqrt{T}$.
\end{description}
\end{distributions}

Le prix à terme est donc multiplié par $\textrm{e}^{\sigma \sqrt{T}}$ pour le prix avec une hausse $S_{u} = F_{t, t + T}\textrm{e}^{\sigma \sqrt{T}}$ et $\textrm{e}^{-\sigma \sqrt{T}}$ pour le prix $S_{d} = F_{t, t + T}\textrm{e}^{-\sigma \sqrt{T}}$ avec une baisse.\\

On isole:
\begin{rappel}{}
	\begin{align*}
	u	&=	\textrm{e}^{(r - \delta)T + \sigma \sqrt{T}}	&
	d	&=	\textrm{e}^{(r - \delta)T - \sigma \sqrt{T}}
	\end{align*}
\end{rappel}

Également, pour ce cas spécial, $p^{*}$ se simplifie à $p^{*}	=	\frac{1}{1 + \textrm{e}^{\sigma \sqrt{T}}}$
