...
% Ce packetage permet de créer des graphiques très intéressantes. Voir en ligne pour avoir des générateur sans avoir à les faire sois-même (le langage est intuitif cependant). 
\usepackage{tikz}
% Packetage pour avoir le symbole du domaine des Z
\usepackage{dsfont}

\definecolor{green_rectangle}{RGB}{131, 176, 84}
\definecolor{red_rectangle}{RGB}{241,112,113}
\definecolor{blue_rectangle}{RGB}{83, 84, 244}
...
%% Pour créer des trêmas sur les signes de prestations avec des couleurs.
...
% j'enlève le footnotesize temporairement, sinon je ne vois rien! G.C.-C.
...
\begin{align*}
\sum_{k = m}^{n} r^k &= \frac{1 - r^{n - m + 1}}{1 - r} &
...
\end{align*}
À noter que la règle est seulement appliquable aux prestations nivelées et donc n'est pas applicable aux prestations (dé)croissantes.
...
\end{align*}
où $\Ex[n]{x}$ est le \textbf{facteur d'actualisation actuarielle}.
...
\subsubsection*{\textcolor{amber(sae/ece)}{Assurances à prestation variant arithmétiquement}}

% Quelques formules de increase qui permet une rapidité dans les calculs (G.C.-C. ou N.L.)
\paragraph{Assurance Vie temporaire croissante} On verse le capital au décès de l'assuré, s'il survient dans les $n$ prochaines années. 


% Possible de générer des graphiques avec le packetage tikz. S'apprends bien mais long à faire manuellement, voir en ligne pour des générateurs de ces graphiques.
% A.J.v.R. 11/10/2019
...
\begin{enumerate}
	\item[] Le capital \textbf{croit chaque année}.\\
	La prestation est de 1 \$ pour la première année, 2 \$ pour la deuxième, etc. jusqu'à n \$ la dernière année.
...
\begin{align*}
	\twoletsymb{I^{(m)}}{\bar{A}}_{\nthtop{1}{x}:\angln} &= \int_0^n \frac{([t \times m] + 1)}{m} v^t \px[t]{x} \mu_{x+t} dt 
\end{align*}

	\item[] Le capital \textbf{croît pendant r années}.\\
\begin{align*}
	\twoletsymb{I_{\anglr}}{\bar{A}}_{\termxn} &= \sum_{k = 0}^{r - 1} \Ax*[k|]{\termxn} \\
%	&= \IA*_{\termxn} + r \Ax*[r|]{\nthtop{1}{x}:\angl{n-r}}
\end{align*}
...
\begin{align*}
	\prescript{}{r|}{\IA*_{\termxn}} &= v^{r}\px[r]{x}\IA*_{\nthtop{1}{x} + r:\angln}
\end{align*}
...
\paragraph{Assurance Vie temporaire décroissante} On verse le capital au décès de l'assuré, s'il survient dans les $n$ prochaines années. 
...
\begin{enumerate}
	...
	On peut le voir de l'autre sens, la prestation du début de la dernière année est de 1 \$, 2 \$ pour le début de l'avant dernière année, etc. 
...
\paragraph{Assurance Vie entière croissante} On verse le capital au décès de l'assuré. 
...
\textbf{\textit{à clarifier}}

Assurance différée et assurance (dé)croissant continûment.
\begin{align*}
...
\end{align*}
\begin{align*}
	\prescript{}{r|}{\DA*_{\termxn}} &= v^{r}\px[r]{x}\DA*_{\nthtop{1}{x} + r:\angln}\\
...
\subsubsection*{\textcolor{amber(sae/ece)}{Assurances à prestation variant exponentiellement}}

La prestation de référence est celle payable en cas de décès immédiat.
La prestation est \textbf{\textit{indexée}} continûment à force $M\tau$; généralement, $\tau < \delta$ et peut même être négatif.\\
...
De plus, l'information est souvent disponible sous format de table et donc les seuls calculs possible sont ceux des contrats payable à la fin de l'année du décès.
